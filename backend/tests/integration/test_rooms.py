...
async def test_list_public_rooms(client: AsyncClient, auth_headers: dict[str, str]):
    ...
    for i in range(3):
        await client.post(
            "/api/v1/rooms",
            json={"name": f"Room {i}", "subject": "Test", "is_public": True},
            headers=auth_headers,
        )
...
async def test_join_room_ok(client: AsyncClient, auth_headers: dict[str, str]):
    ...
    join_resp = await client.post(
        f"/api/v1/rooms/{room_id}/join", headers=joiner_headers
    )
...
async def test_join_full_room(client: AsyncClient, auth_headers: dict[str, str]):
    ...
    await client.post(
        ...
        headers={"Authorization": f"Bearer {login_resp1.json()['access_token']}"},
    )
...
async def test_leave_room_as_member(client: AsyncClient, auth_headers: dict[str, str]):
    ...
    leave_resp = await client.post(
        f"/api/v1/rooms/{room_id}/leave", headers=leaver_headers
    )
...
@pytest.mark.asyncio
async def test_leave_room_as_owner_transfers_ownership(
...
@pytest.mark.asyncio
async def test_leave_room_as_sole_member_deletes_room(
    client: AsyncClient, auth_headers: dict[str, str]
):
    create_resp = await client.post(
        "/api/v1/rooms",
...
async def test_connection_manager_user_still_connected_with_second_socket():
    ...
    mgr = ConnectionManager()
    room_id = uuid.uuid4()
    user = User(id=uuid.uuid4(), email="a@a.com", hashed_password="x", display_name="A")
