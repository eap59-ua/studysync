...
@pytest.mark.asyncio
async def test_create_note_with_valid_pdf(
    client: AsyncClient, auth_headers: dict[str, str], override_notes_service
):
    files = {"file": ("notes.pdf", b"%PDF-1.4...", "application/pdf")}
    data = {"subject": "Calculus", "title": "Derivatives"}

    resp = await client.post(
        "/api/v1/notes", data=data, files=files, headers=auth_headers
    )
...
@pytest.mark.asyncio
async def test_create_note_with_valid_image(
    client: AsyncClient, auth_headers: dict[str, str], override_notes_service
):
...
@pytest.mark.asyncio
async def test_create_note_with_text_file_as_markdown(
    client: AsyncClient, auth_headers: dict[str, str], override_notes_service
):
...
@pytest.mark.asyncio
async def test_create_note_with_invalid_mime_type(
    client: AsyncClient, auth_headers: dict[str, str], override_notes_service
):
...
@pytest.mark.asyncio
async def test_create_note_with_oversized_file(
    client: AsyncClient, auth_headers: dict[str, str], override_notes_service
):
...
@pytest.mark.asyncio
async def test_create_note_with_pdf_having_wrong_magic_bytes(
    client: AsyncClient, auth_headers: dict[str, str], override_notes_service
):
...
@pytest.mark.asyncio
async def test_create_note_can_have_null_room_id(
    client: AsyncClient, auth_headers: dict[str, str], override_notes_service
):
    files = {"file": ("notes.txt", b"Hello world", "text/plain")}
    data = {"subject": "Programming", "title": "Intro"}
...
@pytest.mark.asyncio
async def test_list_notes_filters_by_subject(
    client: AsyncClient, auth_headers: dict[str, str], sample_notes
):
...
@pytest.mark.asyncio
async def test_list_notes_sort_by_rating_desc(
    client: AsyncClient, auth_headers: dict[str, str], sample_notes
):
    ...
    await client.post(
        f"/api/v1/notes/{sample_notes[0]['id']}/reviews",
        json={"rating": 1},
        headers=rev_headers,
    )
    # Review Note 2 with 5 stars
    await client.post(
        f"/api/v1/notes/{sample_notes[2]['id']}/reviews",
        json={"rating": 5},
        headers=rev_headers,
    )
    ...
    # Puede haber notas de otros tests en la BD: filtramos las de esta muestra
...
async def test_list_notes_pagination(
    ...
    resp = await client.get(
        "/api/v1/notes?subject=Pagination&limit=20&page=1", headers=auth_headers
    )
    data = resp.json()
...
@pytest.mark.asyncio
async def test_get_note_returns_reviews_and_avg_rating(
    client: AsyncClient, auth_headers: dict[str, str], sample_notes
):
    ...
    await client.post(
        f"/api/v1/notes/{note_id}/reviews",
...
@pytest.mark.asyncio
async def test_delete_note_as_owner_removes_from_db_and_storage(
    client: AsyncClient,
    auth_headers: dict[str, str],
    memory_storage,
    override_notes_service,
):
    # Create
...
@pytest.mark.asyncio
async def test_delete_note_as_non_owner(
    client: AsyncClient, auth_headers: dict[str, str], sample_notes
):
    ...
    resp = await client.delete(
        f"/api/v1/notes/{sample_notes[0]['id']}", headers=hack_headers
    )
...
@pytest.mark.asyncio
async def test_review_own_note_fails(
    client: AsyncClient, auth_headers: dict[str, str], sample_notes
):
    resp = await client.post(
        f"/api/v1/notes/{sample_notes[0]['id']}/reviews",
...
@pytest.mark.asyncio
async def test_review_twice_fails(
    client: AsyncClient, auth_headers: dict[str, str], sample_notes
):
...
@pytest.mark.asyncio
async def test_review_with_invalid_rating(
    client: AsyncClient, auth_headers: dict[str, str], sample_notes
):
...
async def test_in_memory_storage_save_returns_url_and_key():
    ...
    saved = await storage.save(
        file_bytes=b"test", original_filename="test.txt", content_type="text/plain"
    )
